...
const CryptoFarming = () => {
  ...
  const { id, balance, setBalance, miningPower, miningTotal, setMiningTotal } =
    ...
    if (isFarming && timeRemaining > 0) {
      interval = setInterval(() => {
        ...
        setPointsEarned((prevPoints) => {
          const newPoints =
            ((FARM_DURATION - timeRemaining) / FARM_DURATION) * POINTS_TO_EARN;
            ...
            <div className="w-full flex items-center justify-between space-x-2">
              <button
                ...
                className={`w-[48%] px-4 py-3 flex items-center justify-center text-center rounded-[8px] font-semibold text-[14px] ${
                  isFarming || canClaim ? "bg-btn2 text-[#888]" : "bg-btn"
                }`}
              >
            ...
            <span className="font-medium text-[20px] text-[#ffffff] pt-2 pb-2 flex flex-col justify-center w-full text-center items-center space-x-1">
              <span className="flex items-center justify-center space-x-[2px] text-[18px]">
                ...
                <span className="text-accent pr-[6px]">
                  +{formatNumber(pointsEarned)} 💰MCATS{" "}
                </span>{" "}
                CLAIMED
              </span>
            </span>
        ...
        >
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <p className="pb-6 text-[13px] w-full text-center">
              This is the amount of tokens you earn every 1 hour when mining is
              active, you can boost your mining profit by clicking on the Boost
              Mining option.
            </p>
          </div>
